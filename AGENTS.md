...
## 仕様
- **ChatGPT Plus**のクォーターを前提とし、**ChatGPT5.4-Mini**でのセミエージェント運用を志向する。

## 共通原則

- 文書読込みは、現在の目的タスクを完了するために必要なファイルだけに限定しなければならない。
- LLM は、読んだ文書の記載内容だけで判断しなければならず、推測、補完、慣習、文脈、先回りで意味を追加してはならない。
- 推論でインターネットアクセスが必要になった場合は、処理を中断してユーザに可否を問い合わせなければならない。許可を得ずにインターネットにアクセスしてはならない。
...
- 出力は常に必要最小限に制限しなければならない。冗長な説明、繰り返し、言い換えを含めてはならない。
- 既存情報の再記述は禁止し、差分または新規情報のみを出力しなければならない。
- 出力形式は簡潔な構造（箇条書きまたは短文）を優先しなければならない。
- ログおよび中間結果は要約のみ出力し、詳細な全文展開を行ってはならない。
- 明示的に「詳細」「全文」が指定された場合のみ、これらの制約を一時的に解除できる。
...
- この処理では、assistant 用内容テキストの確定と chat 履歴追記を行わなければならない。
