    ...
    static let outputURL: URL = {
        if let directory = ProcessInfo.processInfo.environment["HELLOWORLD_TEST_METRICS_DIR"],
           !directory.isEmpty {
            ...
            return base.appendingPathComponent("HelloWorldUITests-events.jsonl")
        }
        let bundleID = Bundle.main.bundleIdentifier ?? "HelloWorldUITests"
...
private final class UITestMetricsObserver: NSObject, XCTestObservation {
    ...
    private func appendEvent(
        ...
        let event = UITestMetricsEvent(
            kind: kind,
            testID: testID,
            bundleID: Bundle.main.bundleIdentifier ?? "HelloWorldUITests",
            startedAt: startedAt,
