...
final class AppConsoleLoggerRuntimeLogBuffer {
    private let fileLogLock = NSLock()
    private let runtimeLogLaunchLock = NSLock()
    private var runtimeLogLaunchFileURL: URL?
    private var pendingRuntimeLogLines: [String] = []

    func prepareRuntimeLogFileForLaunch(runtimeLogFileURL overrideURL: URL? = nil) {
    ...
    func writeFileLine(_ line: String) {
        appendRuntimeLogLine(line)
    }

    func activeRuntimeLogFileURL() -> URL? {
#if targetEnvironment(macCatalyst)
        runtimeLogLaunchLock.lock()
    ...
    private func appendRuntimeLogLine(_ line: String) {
#if targetEnvironment(macCatalyst)
    ...
    private func appendPendingRuntimeLogLine(_ line: String) {
    ...
    private func appendPendingRuntimeLogDiagnostic(
        _ event: String,
    ...
    private func writePendingRuntimeLogFlushDiagnosticIfNeeded(
        flushedCount: Int,
        logFileURL: URL,
    ...
    private func writeRuntimeLogLines(_ lines: [String], to logFileURL: URL) throws {
    ...
    private func runtimeLogDiagnosticLine(
        _ event: String,
        level: AppConsoleLogLevel,
        metadata: [String: String]
    ) -> String {
    ...
    private func runtimeLogFailureMetadata(stage: String, logFileURL: URL, error: Error) -> [String: String] {
        let fileManager = FileManager.default
    ...
    private func isLegacyRuntimeLogFileURL(_ url: URL) -> Bool {
