    ...
    static let outputURL: URL = {
        if let directory = ProcessInfo.processInfo.environment["YOUTUBEFEEDER_TEST_METRICS_DIR"],
           !directory.isEmpty {
            ...
            return base.appendingPathComponent("YoutubeFeederTests-events.jsonl")
        }
        let bundleID = Bundle.main.bundleIdentifier ?? "YoutubeFeederTests"
...
private final class TestMetricsObserver: NSObject, XCTestObservation {
    ...
    private func appendEvent(
        ...
        let event = TestMetricsEvent(
            kind: kind,
            testID: testID,
            bundleID: Bundle.main.bundleIdentifier ?? "YoutubeFeederTests",
            startedAt: startedAt,
