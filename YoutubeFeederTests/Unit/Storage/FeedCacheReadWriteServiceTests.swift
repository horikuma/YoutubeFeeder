...
final class FeedCacheReadWriteServiceTests: LoggedTestCase {
    func testLoadRemoteSearchSnapshotReadsCacheWithoutMutatingEntries() async throws {
        ...
        try await withTemporaryFeedCacheBaseDirectory { _ in
            ...
            await remoteCacheStore.save(keyword: "read-snapshot", videos: [CachedVideo(id: "video-remote-read", channelID: channelID, channelTitle: "Remote Read", title: "read snapshot", publishedAt: fetchedAt, videoURL: URL(string: "https://example.com/watch?v=video-remote-read"), thumbnailRemoteURL: nil, thumbnailLocalFilename: nil, fetchedAt: fetchedAt, searchableText: "read snapshot", durationSeconds: 300, viewCount: 20)], totalCount: 1, fetchedAt: fetchedAt)

            let readService = FeedCacheReadService(store: FeedCacheStore(), remoteSearchCacheStore: remoteCacheStore)
    ...
    func testLoadMergedVideosForChannelDoesNotMutateCaches() async throws {
        ...
        try await withTemporaryFeedCacheBaseDirectory { fileManager in
            ...
            database.replaceFeedSnapshot(FeedCacheSnapshot(savedAt: newerDate, channels: [CachedChannelState(channelID: channelID, channelTitle: "Local Channel", lastAttemptAt: olderDate, lastCheckedAt: olderDate, lastSuccessAt: olderDate, latestPublishedAt: olderDate, cachedVideoCount: 1, lastError: nil, etag: nil, lastModified: nil)], videos: [CachedVideo(id: duplicateVideoID, channelID: channelID, channelTitle: "Local Channel", title: "local copy", publishedAt: olderDate, videoURL: URL(string: "https://example.com/watch?v=local"), thumbnailRemoteURL: nil, thumbnailLocalFilename: nil, fetchedAt: olderDate, searchableText: "local copy", durationSeconds: 120, viewCount: 10)]))

            let remoteCacheStore = RemoteVideoSearchCacheStore()
            await remoteCacheStore.save(keyword: "read-service", videos: [CachedVideo(id: duplicateVideoID, channelID: channelID, channelTitle: "Remote Channel", title: "remote copy", publishedAt: newerDate, videoURL: URL(string: "https://example.com/watch?v=remote"), thumbnailRemoteURL: nil, thumbnailLocalFilename: nil, fetchedAt: newerDate, searchableText: "remote copy", durationSeconds: 180, viewCount: 99)], totalCount: 1, fetchedAt: newerDate)
    ...
    func testLoadRefreshStateDoesNotPersistBootstrap() async throws {
        ...
        try await withTemporaryFeedCacheBaseDirectory { fileManager in
            let database = FeedCacheSQLiteDatabase.shared(fileManager: fileManager)
            database.replaceFeedSnapshot(FeedCacheSnapshot(savedAt: now, channels: [CachedChannelState(channelID: channelID, channelTitle: "Refresh Channel", lastAttemptAt: now, lastCheckedAt: now, lastSuccessAt: now, latestPublishedAt: now, cachedVideoCount: 1, lastError: nil, etag: nil, lastModified: nil)], videos: [CachedVideo(id: "video-refresh", channelID: channelID, channelTitle: "Refresh Channel", title: "video", publishedAt: now, videoURL: URL(string: "https://example.com/watch?v=video-refresh"), thumbnailRemoteURL: nil, thumbnailLocalFilename: nil, fetchedAt: now, searchableText: "video", durationSeconds: 600, viewCount: 10)]))

            let readService = FeedCacheReadService(store: FeedCacheStore(), remoteSearchCacheStore: RemoteVideoSearchCacheStore())
