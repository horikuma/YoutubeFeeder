## 2026/03/21
- Human-View の主要クラス図で compact / regular / split detail を束ねる親 View には、`[Adaptive UI]` ではなく `[Layout Variants Host]` を付ける方針に改めた。
  - `Adaptive UI` そのものと、Adaptive UI による表現差分を内包する親実装とは意味が異なるため。図では「表現差分を束ねる親」であることを優先して示す。
