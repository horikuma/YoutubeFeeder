...
## 2026/05/01
- build コマンドは `build/debug` と `build/release` に成果物を出力し、ローカル確認のためコード署名なしで実行する。
  - Debug/Release の成果物を分けて追いやすくし、この環境で再現可能なローカルビルドにするため。

## 2026/04/30

## 2026/04/29
- 分割レイアウトの右ペインは、選択中チャンネルの一覧項目が更新されたら最新キャッシュへ追随し、その完了を診断ログで観測できるようにする。
  - 一覧更新に対して詳細表示が古いまま残る不整合を避け、仕様通りの挙動をログで確認できるようにするため。

## 2026/04/26

## 2026/04/25
- ランタイムログファイル生成失敗時は保留行を保持し、復旧時に同一ログへ書き戻す。
  - PC起動後初回などでログファイル生成が遅延した場合も、通常運用ログだけで失敗経緯と復旧を調査できるようにするため。
- ルール運用意図をSpecs Rulesとして文書化する。
  - 試行回数制約と人間/LLMの役割分担を、実装ルールの背景として追跡できるようにするため。
