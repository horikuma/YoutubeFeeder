...
- 操作:
  - `iPhone` / `iPad` では画面全体の pull-to-refresh、`Mac` では `Refresh` コマンドで手動更新
  - `チャンネル` と `バックアップ` は `Menu`
  - `全設定リセット` は確認ダイアログ経由
  - `キャッシュ検索` は固定キーワード `ゆっくり実況` の検索結果を開く
  - `YouTube検索` は初回表示時に検索せず、遷移先で手動更新した時だけ API 検索する
...
- 操作と遷移:
  ...
  - regular 幅では左ペイン選択で右ペインだけ更新し、選択中チャンネルの一覧項目が更新されたら右ペインの動画一覧も最新キャッシュへ追随する
  - regular 幅では、初めて選んだチャンネルでも右ペインの動画一覧を自動読込する
  - `iPhone` / `iPad` では長押し、`Mac` では左クリックでメニューを開き `チャンネルを削除`
- メニュー:
  - チャンネルタイル: `チャンネルを削除`
...
- 操作と遷移:
  - `iPhone` / `iPad` では pull-to-refresh、`Mac` では `Refresh` コマンドで選択中チャンネルだけ強制更新
  - 分割レイアウトの右ペイン追随時も、再読込完了を診断ログから観測できる
  - 検索結果経由の自動読込中は、画面上部に読込スピナーを表示
  - `iPhone` / `iPad` では長押し、`Mac` では右クリックで動画タイルメニューを開く
  - チャンネル別動画一覧では、`Mac` の左クリックで `YouTubeで開く` を実行する
- メニュー:
  - 動画タイル: `共有`、`チャンネルを削除`
...
- 操作と遷移:
  ...
  - `iPhone` / `iPad` では長押し、`Mac` では左クリックで動画タイルメニューを開く
- メニュー:
  - 動画タイル: `共有`、`チャンネルを削除`
...
- 操作と遷移:
  - `iPhone` / `iPad` では pull-to-refresh、`Mac` では `Refresh` コマンドでキャッシュ検索を再読込
  - 動画タイルの通常タップで `チャンネル別動画一覧画面`
...
- 操作と遷移:
  - 画面表示だけでは検索しない
  - `iPhone` / `iPad` では pull-to-refresh、`Mac` では `Refresh` コマンドで YouTube API 検索
...
- メニュー操作を指す時は `動画タイルメニュー` や `チャンネルタイルメニュー` と呼び、必要なら `iPhone` / `iPad` は長押し、`Mac` は左クリックと添える。
