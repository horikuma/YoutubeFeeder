...
この文書は、人間の開発者が `AGENTS.md` と各 `skills/*.md` を読む前に、開発フローの全体像を短時間でつかむための overview である。正本ではなく、[AGENTS.md](../../AGENTS.md) と [specs.md](../specs.md) を人間向けに読み替えた `human-view` 文書として扱う。
...
```mermaid
flowchart TD
    A["1. AGENTS.md を読む<br/>共通原則と task 入口を確認"] --> B["2. 必要な specs / human-view を読む<br/>変更対象の正本と翻訳資料を絞る"]
...
| --- | --- | --- |
| task 入口 | `AGENTS.md` に並ぶ task 名と対応する `skills/*.md` | いまの依頼でどの skill を読む必要があるか |
| Issue ドリブン進行 | 1 件の指示を Issue 化し、詳細化、実装、検証、文書同期、コミット、PR まで運ぶ流れ | 1 指示を 1 本の Issue と branch に対応付けて追えるか |

## セッション開始時にやること

1. [AGENTS.md](../../AGENTS.md) を入口として読む。
...
| --- | --- | --- | --- |
| 1 | 指示を理解する | ユーザー指示、`AGENTS.md`、必要な specs / code | 変更対象と読取り境界 |
...
- `AGENTS.md` は task の入口と参照順だけを見る文書として読む。
...
| 知りたいこと | 参照先 |
| --- | --- |
| task 入口、共通原則、どの skill を読むか | [AGENTS.md](../../AGENTS.md) |
