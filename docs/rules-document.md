...
- Adaptive UI の細かな実装差分は `CompactView` / `RegularView` / `SplitDetailView` の個別クラスとして列挙せず、親となる機能 View の枠内へ、`[Variants Host]` のような「表現差分を束ねる親」であることが分かる注記を入れて表す。
...
- 所要時間は推測や印象で決めず、`ユーザーが Enter を押して指示を送った直後` に開始時刻を測定し、原則として `コミット直前` に終了時刻を測定して、その差分を分単位へ丸めて記録する。
- コミットしないよう明示された開発シーケンスや、コミットを伴わない応答で終える場合だけは、`最終応答の直前` を終了時刻としてよい。
- 開始時刻と終了時刻の測定には、その場で取得した時刻値を使い、前のコミット時刻、会話の印象、途中の作業メモなどで代用してはならない。
- 日常運用では手計測に頼らず、[llm_elapsed.sh](../scripts/llm_elapsed.sh) を使って `start` と `finish` を記録し、その出力をそのまま貼り付ける。
- `scripts/collect_metrics.sh` による全体計測を行う開発シーケンスでは、計測前に `finish` してはならない。metrics 記録やコミット準備まで含めた開発シーケンス全体を測る。
- 端数の扱いは、秒差を 60 で割った結果を四捨五入して `約x分` とする。
- `未測定` は移行期の例外対応に限り、通常運用では継続させてはならない。連続して発生した場合は手順ではなく運用補助を見直す。
