...
この文書は、HelloWorld プロジェクトの上位方針を定める正本です。ここでは、実装や現行機能の詳細ではなく、長期に維持したい判断基準、開発プロセス、文書運用ルールを扱います。機能仕様は [spec.md](./spec.md)、採用アーキテクチャは [architecture.md](./architecture.md)、詳細設計は [design.md](./design.md) を参照してください。`docs/human-view/` 配下の資料は、人間向けに読みやすく翻訳した参照資料であり正本ではありませんが、人間の開発入口として常時同期対象にします。

## 文書の境界

- `rules.md` には、根幹普遍の方針、開発中の判断基準、開発プロセス、文書運用を記述する。
- [spec.md](./spec.md) には、ユーザー向け機能、画面遷移、操作、表示要件を記述する。
- [architecture.md](./architecture.md) には、プロダクトに依存する採用アーキテクチャ、責務境界、依存方向、データフロー、テスト方針を記述する。
- [design.md](./design.md) には、ファイル単位、型単位、テスト単位の責務や構成など、詳細設計を記述する。
- [gui.md](./human-view/gui.md) には、画面名、GUI パーツ名、画面遷移、指示に使う呼び名を、人間向けの参照資料として整理して記述する。
- [design-overview.md](./human-view/design-overview.md) には、クラス図やレイヤ図などの UML 風設計資料を、人間向けの参照資料として整理して記述する。
- `rules.md` へ実装詳細や画面単位の仕様を集約してはならない。
- 文書の内容が詳細化しすぎた場合は、上位方針を残して詳細を `architecture.md`、`design.md`、または `spec.md` へ移す。
...
- 採用アーキテクチャや UI フレームワークに依存する設計は `architecture.md` で定義し、`rules.md` にはその土台となる責務分離の原則だけを置く。
...
- 詳細設計やテスト配置の変更では [design.md](./design.md) を更新する。
- 上位方針や運用変更では `rules.md` を更新する。
- 人間向けの参照性に影響する変更では [gui.md](./human-view/gui.md) と [design-overview.md](./human-view/design-overview.md) を更新する。
