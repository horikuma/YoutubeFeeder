...
この文書は、HelloWorld プロジェクトの上位方針を定める正本である。ここでは、実装や現行機能の詳細ではなく、長期に維持したい判断基準、開発プロセス、文書運用ルールを扱う。機能仕様は [spec.md](./spec.md)、採用アーキテクチャは [architecture.md](./architecture.md)、詳細設計は [design.md](./design.md) を参照する。`docs/human-view/` 配下の資料は、人間向けに読みやすく翻訳した参照資料であり正本ではないが、人間の開発入口として常時同期対象とする。
...
- `rules.md`
  - 根幹普遍の方針、開発中の判断基準、開発プロセス、文書運用を記述する。
- [spec.md](./spec.md)
