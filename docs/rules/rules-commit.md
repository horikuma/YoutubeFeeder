...
- コミットとは、その時点で完了条件を満たした変更セットを 1 単位として確定し、履歴と対応関係が追跡できる状態へ整えるタスクである。
- このタスクでは、コミット対象の確定、`docs/history/` の必要更新、Git の staging、commit message、完了確認までをこの文書だけで判断しなければならない。
...
- コミット前には、変更セットに応じて必要な検証と文書更新が完了していることを確認しなければならない。
- ソースコード変更を含む場合は、ユーザーから明示的に停止されていない限り、必要な検証、文書更新、計測記録を終えてからコミットしなければならない。
- ドキュメントだけを変更した場合も、ユーザーから明示的に停止されていない限り、その変更セットに必要な文書更新と当日履歴更新を終えてからコミットしなければならない。
...
- 必要な検証、文書更新、当日履歴更新が完了したうえでコミットされていること。
