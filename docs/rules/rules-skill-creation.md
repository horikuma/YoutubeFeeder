...
- `scripts` や `_meta.json` を rules で説明する場合は、`./scripts/command-name --option {variable}` のように、そのまま実行できる形へ一意に展開できる command 形式と、参照する `_meta.json` や `llm-cache` のキー名だけを記述しなければならない。
...
- LLM は rules や Issue コメントに command 例を書く時、山括弧による置換記法のように文字列そのものとして送信されうる置換記法を使ってはならず、`{repo_slug}` のように変数であることが明確な記法だけを使わなければならない。
