# User Instruction Understanding Rules

この文書は、ユーザー指示の理解タスクを単体で定義する完結文書である。
...
- rules に command 例や置換記法を追加する場合は、`./scripts/... --option {variable}` のように、そのまま実行できる形へ一意に展開できる形で記述しなければならず、山括弧による置換記法のように文字列そのものとして送信されうる表記を残してはならない。
