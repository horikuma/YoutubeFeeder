...
- 最終の全体テストでは、`scripts/metrics-collect` を使って build 時間、全体 test 時間、起動性能、`docs/metrics/metrics-test.md` を同じ 1 回の全体実行から取得しなければならない。
...
- `scripts/metrics-collect` を使うべき最終全体確認を別手段で代替してはならない。
