...
- この処理では、開始時タイムスタンプ記録、`docs/history/chat-latest.md` へ追記するための 1 行のユーザー指示確定、その追記完了までをこの文書だけで判断しなければならない。
...
- チャット入力は、その後続タスクで参照する正本の 1 行 `user_line` へ確定しなければならない。
...
- `user_line` は、先頭を `- ` で始めた 1 行のユーザー指示として確定しなければならない。
- `docs/history/chat-latest.md` への追記は、次の usage で `./scripts/command-runner.py 'history-chat-append'` を使わなければならない。
  ...
  例: `./scripts/command-runner.py 'history-chat-append' --user-line '- セッションを開始せよ。'`
  - `<user_line>` は、この文書で確定した 1 行のユーザー指示であり、省略してはならない。
...
- 後続タスクで参照する `user_line` が 1 行へ確定していること。
...
- `user_line` を確定しないまま `docs/history/chat-latest.md` へ追記してはならない。
