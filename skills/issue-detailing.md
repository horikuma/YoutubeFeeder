...
- 中間成果物は、後続フェーズの判断に必要な情報だけを含めなければならない。同一内容の再記述、言い換え、補足説明を含めてはならない。
- 各フェーズの結果は、中間成果物として明示的に出力しなければならない。ただし、出力は意味を保持した最小表現（箇条書き、構造化形式）とし、冗長な説明文を含めてはならない。
...
- [ ] 4. `scripts/xcode-build` 配下 command の `./scripts/command-runner.py 'build-debug'` または `./scripts/command-runner.py 'build-release'` を実行し、成功することを確認する。
```
...
- Issue コメント本文には、`./scripts/command-runner.py 'issue-comment-create'` が検証する必須見出しとして、次の 4 行を行頭から完全一致で含めなければならない。
  - `## 私の指示`
  - `## その時点で行なった判断`
  - `## 最終的に最新になった禁止事項`
  - `## 最終的に最新になった ToDo`
- 上記 4 行を、`# 私の指示`、`# その時点で行なった判断`、`# 最終的に最新になった禁止事項`、`# 最終的に最新になった ToDo`、`### 私の指示`、`### その時点で行なった判断`、`### 最終的に最新になった禁止事項`、`### 最終的に最新になった ToDo` として出力してはならない。
- Issue コメント本文に `# 読取り結果`、`# 抽出結果`、`# 正規化結果` を含める場合でも、上記 4 行は `##` 見出しとして別途出力しなければならない。
