...
- ブランチに関しては、`./scripts/command-runner.py 'git-main-sync'` で `main` を最新化した後、`git branch -D` で Squash マージ済みの local branch を掃除することだけを目的とする。
...
- Squash マージ済み local branch 掃除では、次の command だけを正規手順として使わなければならない。
  `git branch -D <branch_name>`
  例: `git branch -D issue-80`
  - `<branch_name>` は、`main` 以外の local branch のうち、対応 Pull Request が Squash マージで取り込まれ、local branch を強制削除してよい branch 名でなければならない。
- Issue 駆動で進めた作業の `LLM所要時間` 反映では、次の command だけを正規手順として使わなければならない。
  ...
  - `<minutes>` は、そのセッション開始から session-end 実行時点までの累計 `LLM所要時間` 分数であり、省略してはならない。
- セッション終了タスクは、次の 4 条件を満たした時だけ完了とみなす。

## 完了条件

- `./scripts/command-runner.py 'git-main-sync'` が終了コード `0` で成功し、その結果として `main` が最新化されていること。
- `main` の最新化後に、`git branch -D` で削除対象の `main` 以外の local branch の掃除が完了していること。
- Issue 駆動で進めた作業では、`./scripts/command-runner.py 'project-field-set' --issue-number '<issue_number>' --value '<minutes>'` が成功し、対象 Issue へそのセッション累計の `LLM所要時間` が反映されていること。
- 上記 3 条件の結果として、終了時の基準ブランチが `main` になっており、次セッションへ不要な `LLM所要時間` 累積を持ち越さない状態になっていること。
...
- Squash マージ済みでない local branch の `git branch -D` を、セッション終了へ含めてはならない。
