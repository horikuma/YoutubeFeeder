...
- ブランチに関しては、`./scripts/command-runner.py 'git-main-sync'` で `main` を最新化した後、`./scripts/command-runner.py 'git' fetch --prune` で remote-tracking branch を掃除することだけを目的とする。
...
- remote-tracking branch 掃除では、次の command だけを正規手順として使わなければならない。
  `./scripts/command-runner.py 'git' fetch --prune`
  例: `./scripts/command-runner.py 'git' fetch --prune`
...
- `main` の最新化直後に、`./scripts/command-runner.py 'git' fetch --prune` が成功していること。
...
- merged な local branch の削除や、`./scripts/command-runner.py 'git' branch -d` / `./scripts/command-runner.py 'git' branch -D` による local branch 操作をセッション開始へ含めてはならない。
...
- 履歴移動後の差分確認が必要な場合は、`./scripts/command-runner.py 'git'` を通して確認しなければならない。
