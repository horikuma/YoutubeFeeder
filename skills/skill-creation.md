...
- `scripts` や `_meta.json` を rules で説明する場合は、usage 記法、具体的な command 例文、usage だけでは確定しない `_meta.json` や `llm-cache` のキー名だけを記述しなければならない。
...
- LLM は rules や Issue コメントに command 例を書く時、usage 記法、各置換値の説明、例えば `./scripts/command-runner.py 'issue-read' --issue-number '57'` のような具体的な command 例文を使わなければならず、説明なしの置換記法を残してはならない。
